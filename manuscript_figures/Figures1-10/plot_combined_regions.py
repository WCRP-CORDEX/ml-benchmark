...
Modes (``--mode``):
    ...
    daily          Single-day snapshot (wettest for pr / hottest for tasmax)
                   chosen by area-mean on the truth.
"""
...
def _region_band(subfig, domain, gcm, *, args, paths, rankings, mode):
    ...
    available = bu.discover_available(
        ...
        period_key=args.period, gcm=gcm,
    ...
    # Pick the truth field for this band
    if mode == "climatology":
        truth_field = bu.climatology(truth, args.variable)
        title_index = INDEX_NAME[args.variable]
        date_hint = None
    else:  # daily
        mode_k = "hottest" if args.variable == "tasmax" else "wettest"
        day = bu.find_extreme_day(truth, mode=mode_k)
    ...
    # Pred fields for the 6 sampled models
    ...
    if mode == "climatology":
        ...
        for mname in available:
            try:
                f = _field_for(mname)
                all_vals[mname] = float(bu.rmse(f, truth_field))
            except Exception as e:
                warnings.warn(f"[{domain}] RMSE skip {mname}: {e}")
        truth_scalar = None
    else:
        # All-model spatial max on the day
        for mname in available:
            try:
                da = bu.open_prediction(paths, mname, domain, args.experiment,
                                        args.period, gcm, args.variable,
    ...
    if args.variable == "pr":
        vmin = 0.0
        vmax = max(float(np.nanpercentile(stack, 99.95 if mode == "daily" else 99)), 5.0)
    else:
        vmin = float(np.nanpercentile(stack, 1))
        vmax = float(np.nanpercentile(stack, 99))
    if mode == "climatology":
        cmap = bu.cmap_for(args.variable, kind="climatology")
    else:
        cmap = bu.cmap_for(args.variable, kind="daily")
    ...
    subfig.text(0.032, 0.57, domain, rotation=90,
                fontsize=22, fontweight="bold", style="italic",
                color="#222", va="center", ha="center")
    # TOP / BOTTOM
    subfig.text(0.044, 0.72, "TOP", rotation=90, fontsize=20,
                fontweight="bold", color="#1a5e2a", va="center", ha="center")
    subfig.text(0.044, 0.42, "BOTTOM", rotation=90, fontsize=20,
    ...
    def small_panel(row, col, field, model, rank_label, letter):
        ...
        ax.text(0.02, 0.97, letter, transform=ax.transAxes,
                ...
                color="k", path_effects=[_stroke(1.3, "#000")])
        ax.set_title(f"{rank_label}   {model}",
                     fontsize=17, pad=3)
        if mode == "climatology":
    ...
    units = bu.units_for(args.variable)
    cb.set_label(f"{bu.pretty_var(args.variable)}  [{units}]",
...
def _draw_rmse_kde_inset(ax, all_rmse, tops, bots, variable):
    ...
    names = list(all_rmse); vals = np.asarray([all_rmse[n] for n in names], float)
    ok = np.isfinite(vals); names = [n for n, k in zip(names, ok) if k]; vals = vals[ok]
    if vals.size == 0:
        ax.set_axis_off(); return
    ...
    ax.set_xlabel(f"RMSE  [{bu.units_for(variable)}]",
...
def _draw_max_kde_inset(ax, all_max, truth_max, tops, bots, variable):
    if not all_max:
        ax.set_axis_off(); return
    names = list(all_max); vals = np.asarray([all_max[n] for n in names], float)
    ok = np.isfinite(vals); names = [n for n, k in zip(names, ok) if k]; vals = vals[ok]
    if vals.size == 0:
        ax.set_axis_off(); return
    ...
    ax.set_ylabel("Density", fontsize=13)
    ax.tick_params(labelsize=12)
    ax.set_title(f"Max across {vals.size} models", fontsize=11, pad=3,
                 fontweight="bold")
    ax.grid(axis="y", linewidth=0.3, alpha=0.5); ax.set_axisbelow(True)
...
def main() -> None:
    ...
    title_index = (INDEX_NAME[args.variable] if args.mode == "climatology"
                   else "Case studies (historical)")
    fig.suptitle(
        f"{args.variable.upper()}  ·  {args.mode}\n  "
        f"{args.experiment.replace('_', ' ')}  ·  {title_index}",
        fontsize=29, fontweight="bold", y=1.05,
    )
