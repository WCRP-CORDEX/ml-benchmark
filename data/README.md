...
Training data is provided for the two benchmark experiments focusing on the standard (ESD) and emulation downscaling approaches, denoted `ESD Pseudo-Reality` and `Emulator Hist+Future`. Test data includes both `perfect` (upscaled from the RCM) and `imperfect` (from the driving GCM) predictors both with ~150km resolution. Predictands (target for training) correspond to the RCM highres ~10km temperature and precipitation output.
